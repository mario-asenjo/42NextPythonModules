...
def main() -> None:
    ...
    exclusive_alice: set = alice_achievements.difference(bob_achievements.union(charlie_achievements))
    exclusive_bob: set = bob_achievements.difference(alice_achievements.union(charlie_achievements))
    exclusive_charlie: set = charlie_achievements.difference(alice_achievements.union(bob_achievements))
    rare_achievements: set = exclusive_alice.union(exclusive_bob, exclusive_charlie)
    print("Rare achievements (1 player):", rare_achievements)
    alice_vs_bob_common: set = alice_achievements.intersection(bob_achievements)
    print("\nAlice vs Bob common:", alice_vs_bob_common)
    alice_unique_vs_bob: set = alice_achievements.difference(bob_achievements)
    print("Alice unique:", alice_unique_vs_bob)
    bob_unique_vs_alice: set = bob_achievements.difference(alice_achievements)
