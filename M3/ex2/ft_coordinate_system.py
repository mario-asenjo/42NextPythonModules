...
def calculate_distance(x1: int, y1: int, z1: int, x2: int, y2: int, z2: int) -> float:
    return math.sqrt((x2 - x1)**2 + (y2 - y1)**2 + (z2 - z1)**2)


def parse_arguments(args: list[str], pos: int) -> tuple:
    start: tuple[int, int, int]
    try:
        start = (int(args[pos].split(',')[0]), int(args[pos].split(',')[1]), int(args[pos].split(',')[2]))
        return start
...
def main(args: list[str]) -> None:
    ...
    print(f"Distance between {player} and {position}: %.2f" % calculate_distance(*player, *position))
    if argc == 3:
