...
def main() -> None:
    ...
    try:
        ...
        with open('ancient_fragment.txt', 'r') as file:
            ...
            fragments: list[str] = ['{[}%s{]}' % line[line.find('[') + 1 : line.find(']')] for line in data]
            sentences: list[str] = [line[line.find(']') + 1 :] for line in data]
            full_sentences: list[str] = [f"{fr}{sent}" for fr, sent in zip(fragments, sentences)]
            print("\nRECOVERED DATA:")
