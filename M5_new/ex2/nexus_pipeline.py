...
class JSONAdapter(ProcessingPipeline):
    ...
    def process(self, data: Any) -> str:
        ...
        result: str = (
            f"Processed temperature reading: {value}ºC "
            f"({enriched['status']})"
        )
...
class StreamAdapter(ProcessingPipeline):
    ...
    def process(self, data: Any) -> str:
        ...
        result: str = f"Stream summary: {count} readings, avg: {average:.1f}ºC"
        print(f"Output: {result}\n")
        return result
...
class NexusManager:
    ...
    def recover_with_backup(
        ...
        """Try primary pipeline, then fall back to back up"""
        try:
            return self.run_pipeline(primary, data)
        except ValueError:
            print("Recovery initiated: Switching to backup processor")
            result: str = self.run_pipeline(backup, data)
            print("Recovery successfull: Pipeline restored, processing resumed\n")
            return result

    def get_pipeline_stats(self, pipeline_id: int) -> Dict[str, float]:
        """Return status for a specific pipeline"""
